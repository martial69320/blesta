...
class Solusvm extends Module
{
    ...
    public function editService($package, $service, array $vars = [], $parent_package = null, $parent_service = null)
    {
        ...
        if ($vars['use_module'] == 'true') {
            ...
            if (isset($delta['solusvm_template']) && isset($vars['confirm_reinstall']) && $vars['confirm_reinstall']) {
                $data = ['template' => $delta['solusvm_template']];
    ...
    public function getPackageFields($vars = null)
    {
        ...
        $total_base_ip_addresses->attach(
            $fields->fieldText(
                ...
                $this->Html->ifSet($vars->meta['total_base_ip_addresses'], 1),
        ...
        $set_node->attach(
            $fields->fieldRadio(
                'meta[set_node]',
                '0',
                $this->Html->ifSet($vars->meta['set_node'], '0') == '0',
        ...
        $set_node->attach(
            $fields->fieldRadio(
                'meta[set_node]',
                '1',
                $this->Html->ifSet($vars->meta['set_node']) == '1',
        ...
        $node_group->attach(
            $fields->fieldSelect(
                'meta[node_group]',
                $groups,
                $this->Html->ifSet($vars->meta['node_group']),
    ...
    public function getEmailTags()
    {
        return [
            'module' => ['host', 'port'],
            'package' => [],
            'service' => ['solusvm_vserver_id', 'solusvm_console_user', 'solusvm_console_password',
    ...
    public function getAdminAddFields($package, $vars = null)
    {
        ...
        $host_name->attach(
            $fields->fieldText(
                'solusvm_hostname',
                $this->Html->ifSet($vars->solusvm_hostname),
                ['id' => 'solusvm_hostname']
            )
        );
        ...
        if (isset($package->meta->set_template) && isset($package->meta->type)
            ...
            $template->attach(
                $fields->fieldSelect(
                    'solusvm_template',
                    $templates,
                    $this->Html->ifSet($vars->solusvm_template),
                    ['id' => 'solusvm_template']
                )
            );
            // Set the label as a field
        ...
        $vserver_id->attach(
            $fields->fieldText(
                'solusvm_vserver_id',
                $this->Html->ifSet($vars->solusvm_vserver_id),
    ...
    public function getClientAddFields($package, $vars = null)
    {
        ...
        $host_name->attach(
            $fields->fieldText(
                'solusvm_hostname',
                $this->Html->ifSet($vars->solusvm_hostname, $this->Html->ifSet($vars->domain)),
                ['id' => 'solusvm_hostname']
            )
        );
        // Set the label as a field
    ...
    {
        return [
            'tabClientActions' => ['name' => Language::_('Solusvm.tab_actions', true), 'icon' => 'fa fa-gears'],
            'tabClientStats' => ['name' => Language::_('Solusvm.tab_stats', true), 'icon' => 'fa fa-bar-chart'],
            'tabClientConsole' => ['name' => Language::_('Solusvm.tab_console', true), 'icon' => 'fa fa-terminal'],
            'tabClientIps' => ['name' => Language::_('Solusvm.tab_ips', true), 'icon' => 'fa fa-cog']
        ];
    }
    ...
    public function validateHostName($host_name, $require_fqdn = false)
    {
        ...
        $nested_octet = "(\." . $octet . ')';
        $hostname_regex = '/^' . $octet . $nested_octet . ($require_fqdn ? $nested_octet : '') . '+$/i';
