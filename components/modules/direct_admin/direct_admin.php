...
class DirectAdmin extends Module
{
    ...
    public function getPackageFields($vars = null)
    {
        ...
        $type->attach(
            $fields->fieldRadio(
                'meta[type]',
                'user',
                $this->Html->ifSet($vars->meta['type'], 'user') == 'user',
        ...
        if ($module_row) {
            ...
            $command = ($this->Html->ifSet($vars->meta['type']) == 'reseller'
                ? 'getPackagesReseller'
                : 'getPackagesUser'
            );
     ...
     *
     * @return array A multi-dimensional array of key/value pairs where each
     *  key is one of 'module', 'package', or 'service' and each value is a
     *  numerically indexed array of key values that match meta fields under that category.
    ...
    public function tabClientActions($package, $service, array $get = null, array $post = null, array $files = null)
    {
        ...
        if (!empty($post)) {
            ...
            $data = [
                'direct_admin_password' => $this->Html->ifSet($post['direct_admin_password']),
                'direct_admin_confirm_password' => $this->Html->ifSet($post['direct_admin_confirm_password']),
            ];
    ...
    public function getAdminAddFields($package, $vars = null)
    {
        ...
        $domain->attach(
            $fields->fieldText(
                'direct_admin_domain',
                $this->Html->ifSet($vars->direct_admin_domain),
        ...
        $username->attach(
            $fields->fieldText(
                'direct_admin_username',
                $this->Html->ifSet($vars->direct_admin_username),
        ...
        $password->attach(
            $fields->fieldPassword(
                ...
                [
                    ...
                    'value' => $this->Html->ifSet($vars->direct_admin_password)
                ]
            )
        );
        // Set the label as a field
        ...
        $fields->setHtml('<a class="generate-password"
            ...
            <i class="fa fa-refresh"></i> ' . Language::_('DirectAdmin.service_field.text_generate_password', true) .
        '</a>
        <script type="text/javascript">
            $(document).ready(function () {
        ...
        $email->attach(
            $fields->fieldText(
                'direct_admin_email',
                $this->Html->ifSet($vars->direct_admin_email),
    ...
    public function getClientAddFields($package, $vars = null)
    {
        ...
        $domain->attach(
            $fields->fieldText(
                'direct_admin_domain',
                $this->Html->ifSet($vars->direct_admin_domain, $this->Html->ifSet($vars->domain)),
                ['id' => 'direct_admin_domain']
            )
        );
        // Set the label as a field
