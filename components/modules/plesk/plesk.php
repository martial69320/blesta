...
class Plesk extends Module
{
     ...
     *
     * @return array A numerically indexed array that contains an array with key/value pairs
     *  for 'name' and 'url', representing the name and URL of the authors of this module
     */
            ...
            if ($update_plan['from_plan'] != $update_plan['to_plan']) {
                ...
                if ($update_plan['reseller']) {
                    try {
                        ...
                        $this->log(
                            $module_row->meta->ip_address . '|reseller:switch-subscription',
                            serialize($data),
                            'input',
                            true
                        );
                ...
                if ($package_from->meta->plan != $package_to->meta->plan) {
                    ...
                    try {
                        ...
                        $this->log(
                            $module_row->meta->ip_address . '|webspace:switch-subscription',
                            serialize($data),
                            'input',
                            true
                        );
    ...
    {
        $meta_fields = ['server_name', 'ip_address', 'port', 'username', 'password', 'panel_version', 'reseller'];
    ...
    {
        return ['first'=>Language::_('Plesk.order_options.first', true)];
    }
        ...
        if (isset($vars->meta['type']) && $vars->meta['type'] == 'reseller') {
            ...
            $package->attach(
                $fields->fieldSelect(
                    'meta[reseller_plan]',
                    $reseller_plans,
                    $this->Html->ifSet($vars->meta['reseller_plan']),
    ...
    {
        return [
            'module' => ['ip_address', 'port'],
            'package' => ['type', 'plan', 'reseller_plan'],
            'service' => ['plesk_domain', 'plesk_username', 'plesk_password', 'plesk_webspace_id']
        ];
    }
    ...
    public function getAdminAddFields($package, $vars = null)
    {
        ...
        $fields = $this->getAdminServiceFields();
        ...
        $webspace_id->attach(
            $fields->fieldText(
                'plesk_webspace_id',
                $this->Html->ifSet($vars->plesk_webspace_id),
    ...
    public function getClientAddFields($package, $vars = null)
    {
        ...
        $domain->attach(
            $fields->fieldText(
                'plesk_domain',
                $this->Html->ifSet($vars->plesk_domain, $this->Html->ifSet($vars->domain)),
                ['id' => 'plesk_domain']
            )
        );
        // Set the label as a field
    ...
    private function getAdminServiceFields($vars = null)
    {
        ...
        $password_options = $this->Json->encode((object)['include' => $password_options]);
        ...
        // Create domain field and attach to domain label
        $domain->attach(
            $fields->fieldText('plesk_domain', $this->Html->ifSet($vars->plesk_domain), ['id'=>'plesk_domain'])
        );
        // Set the label as a field
        $fields->setField($domain);
        ...
        $username->attach(
            $fields->fieldText('plesk_username', $this->Html->ifSet($vars->plesk_username), ['id'=>'plesk_username'])
        );
        // Add tooltip
        ...
        $fields->setHtml('<a class="generate-password"
            ...
            <i class="fa fa-refresh"></i> ' . Language::_('Plesk.service_field.text_generate_password', true) .
        '</a>
        <script type="text/javascript">
            $(document).ready(function () {
        ...
        $password->attach(
            $fields->fieldPassword(
                ...
                [
                    ...
                    'value' => $this->Html->ifSet($vars->plesk_password)
                ]
            )
        );
        // Set the label as a field
    ...
    private function upgradeCustomerToReseller($module_row, $service)
    {
        ...
        try {
            ...
            $this->log(
                $module_row->meta->ip_address . '|customer:convert-to-reseller',
                serialize($data),
                'input',
                true
            );
