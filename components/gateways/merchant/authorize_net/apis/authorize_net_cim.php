            ...
            case 'ach':
                ...
                $payment = [
                    'bankAccount' => [
                        ...
                        'nameOnAccount' => $this->ifSet($account_info['first_name'])
                            . ' ' . $this->ifSet($account_info['last_name']),
                        'echeckType' => 'WEB'
                    ]
                ];
                break;
            case 'cc':
                $payment = [
                    'creditCard' => [
                        ...
                        'expirationDate' => substr($this->ifSet($account_info['card_exp']), 0, 4)
                            . '-' . substr($this->ifSet($account_info['card_exp']), -2)
                    ]
                ];
    ...
    private function createProfileTransaction(
        ...
        if ($invoice_amounts) {
            $data['transaction'][$trans_type]['order']['invoiceNumber']
                = $this->ifSet($invoice_amounts[0]['invoice_id']);
        }
    ...
    private function getPaymentProfile($profile_id, $payment_profile_id)
    {
        ...
        if (isset($response_obj->messages->resultCode) &&
            ...
            if (isset($response_obj->paymentProfile->payment->creditCard)) {
                $result['creditCard'] = [
                    'cardNumber' => (string)$this->ifSet(
                        $response_obj->paymentProfile->payment->creditCard->cardNumber
                    ),
                    'expirationDate' => (string)$this->ifSet(
