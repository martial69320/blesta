...
class PaypalPaymentsStandard extends NonmerchantGateway
{
    ...
    {
        return ['AUD', 'BRL', 'CAD', 'CZK', 'DKK', 'EUR', 'HKD', 'HUF', 'ILS', 'JPY',
    ...
    public function validate(array $get, array $post)
    {
        ...
        if ($response != 'VERIFIED' || (
            ...
            if ($response === 'INVALID') {
                ...
                // Log error response
                $this->log($this->ifSet($_SERVER['REQUEST_URI']), $response, 'output', false);
                return;
            }
        }
        ...
        switch (strtolower($this->ifSet($post['payment_status']))) {
            ...
            default:
                ...
                $this->log($this->ifSet($_SERVER['REQUEST_URI']), $post['payment_status'], 'output', $verified);
        ...
        return [
            ...
            'status' => $status,
            'reference_id' => $this->ifSet($post['payer_email']),
