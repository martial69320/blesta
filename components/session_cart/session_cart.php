...
class SessionCart
{
    ...
    {
        #
        # TODO: the use of the Events component and the EventObject has been @deprecated since 4.3.0
        # in favor of the /Blesta/Core/Util/Events/Common/EventInterface, which the SessionCart should
        # use exclusively once the Events component is removed
        #
        // Load the event handler
        Loader::loadComponents($this, ['Events']);
    ...
    public function addItem($item)
    {
        ...
        // Trigger the event
        $event = new EventObject('SessionCart.' . $this->cart_name . '.addItem', ['item' => $item, 'index' => $i]);
    ...
    {
        // Trigger the event
        $event = new EventObject('SessionCart.' . $this->cart_name . '.prequeueItem', ['item' => $item]);
        $this->Events->trigger($event);
