...
class Contacts extends AppModel
{
    ...
    public function add(array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventListener->register('Contacts.add');
            $eventListener->trigger(
                $eventFactory->event('Contacts.add', ['contact_id' => $contact_id, 'vars' => $vars])
            );
     ...
     */
    private function getRequiredFieldRules($client_id)
    {
    ...
    public function edit($contact_id, array $vars)
    {
        ...
        $rules['contact_type']['inv_address_to'] = [
            'if_set' => true,
            'rule' => [[$this, 'validateReceivesInvoices'], $this->ifSet($contact_id, null)],
            'negate' => true,
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('Contacts.edit');
            $eventListener->trigger(
                $eventFactory->event(
                    'Contacts.edit',
    ...
    public function delete($contact_id)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $this->Record->from('contacts')
                ...
                ->delete(['contacts.*', 'contact_numbers.*', 'contact_permissions.*']);

            // Trigger the event
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('Contacts.delete');
            $eventListener->trigger(
                $eventFactory->event('Contacts.delete', ['contact_id' => $contact_id, 'old_contact' => $contact])
            );
        }
    }
    ...
    private function getRules(array $vars, $edit = false)
    {
        ...
        $rules = [
            ...
            'contact_type' => [
                'format' => [
                    ...
                    'rule' => [
                        ...
                        $this->ifSet($vars['contact_id']),
                        $this->ifSet($vars['client_id'])
                    ],
                    ...
                    'rule' => [
                        function ($email, $client_id, $contact_type, $contact_id) {
                            ...
                            if ($client) {
                                ...
                                $contact_email = $this->ifSet($setting['value'], $contact_email);
                            }
            ...
            'state' => [
                ...
                'country_exists' => [
                    'if_set' => true,
                    'rule' => [[$this, 'validateStateCountry'], $this->ifSet($vars['country'])],
