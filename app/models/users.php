    ...
    public function login(Session $session, array $vars)
    {
        ...
        if (($user_id = $session->read('blesta_auth')) != '') {
            ...
            $rules = [
                'username' => [
                    'attempts' => [
                        'rule' => [[$this, 'validateLoginAttempts'], $this->ifSet($vars['ip_address'])],
    ...
    public function edit($user_id, array $vars, $validate_pass = false)
    {
        ...
        if ($validate_pass) {
            $rules['current_password'] = [
                'matches' => [
                    'rule' => [[$this, 'validatePasswordEquals'], $user_id],
    ...
    private function getRules(array $vars, $edit = false)
    {
        $rules = [
            'username' => [
                ...
                'unique' => [
                    'rule' => [[$this, 'validateUniqueUser'], $this->ifSet($vars['user_id'])],
            ...
            'new_password' => [
                ...
                'matches' => [
                    'rule' => ['compares', '==', $this->ifSet($vars['confirm_password'])],
                    'message' => $this->_('Users.!error.new_password.matches'),
                    'post_format' => [[$this, 'hashPassword']]
                ]
            ],
            'two_factor_mode' => [
                'format' => [
            ...
            'two_factor_key' => [
                'valid' => [
                    'if_set' => true,
                    'rule' => [[$this, 'validateTwoFactorKey'], $this->ifSet($vars['two_factor_mode'])],
