...
class Staff extends AppModel
{
    ...
    public function add(array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            // Add staff
            $fields = ['user_id', 'first_name', 'last_name', 'email', 'email_mobile', 'status'];
            ...
            $eventListener->register('Staff.add');
            $eventListener->trigger($eventFactory->event('Staff.add', ['staff_id' => $staff_id, 'vars' => $vars]));
    ...
    public function edit($staff_id, array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            // Update staff
            $fields = ['first_name', 'last_name', 'email', 'email_mobile', 'status'];
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('Staff.edit');
            $eventListener->trigger(
                $eventFactory->event('Staff.edit', ['staff_id' => $staff_id, 'vars' => $vars, 'old_staff' => $staff])
            );
        }
    }
    ...
    public function sendNotificationEmail(
        ...
        foreach ($staff as $staff_member) {
            ...
            if (isset($tags['package'])) {
                ...
                foreach ($this->ifSet($tags['package']->names, []) as $name) {
                    if ($name->lang == $lang) {
                ...
                foreach ($this->ifSet($tags['package']->descriptions, []) as $description) {
    ...
    private function getAddRules(array $vars)
    {
        $rules = [
            ...
            'groups' => [
                'unique_company' => [
                    'rule' => [[$this, 'validateUniqueCompanies'], $this->ifSet($vars['groups'])],
    ...
    private function getNoticeRules(array $vars)
    {
        $rules = [
            ...
            'action' => [
                'exists' => [
                    'rule' => [[$this, 'validateNoticeActionExists'], $this->ifSet($vars['staff_group_id'])],
