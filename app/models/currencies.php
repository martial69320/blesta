...
class Currencies extends AppModel
{
    ...
    public function convert($amount, $from_currency, $to_currency, $company_id)
    {
        ...
        if (($from = $this->get($from_currency, $company_id)) && (($to = $this->get($to_currency, $company_id)))) {
            ...
            $from_amount = $this->toDecimal($amount, $from->code, $company_id, $from->precision);
            $to_amount = ($to->exchange_rate == 0) ? 0 : ($from_amount * ($to->exchange_rate / $from->exchange_rate));
            return $this->toDecimal($to_amount, $to->code, $company_id, $to->precision);
        }
        return $amount;
    }
    ...
    public function toDecimal($value, $currency, $company_id = null, $decimals = null)
    {
        ...
        if ($formats[$currency][$company_id]) {
            ...
            $value = str_replace(
                [
                    $formats[$currency][$company_id]->prefix,
                    $formats[$currency][$company_id]->suffix
                ],
                '',
                $value
            );
    ...
    private function getRules($vars)
    {
        $rules = [
            'code' => [
                ...
                'exists' => [
                    'rule' => [[$this, 'validateCurrencyExists'], $this->ifSet($vars['company_id'])],
                    'negate' => true,
                    'message' => $this->_(
                        'Currencies.!error.code.exists',
                        $this->ifSet($vars['code']),
    ...
    public function updateRates()
    {
        ...
        $rate_processor = $this->getExchangeRateProcessor($this->ifSet($company_settings['exchange_rates_processor']));
        ...
        if ($rate_processor !== false) {
            ...
            if ($rate_processor->requiresKey()) {
                $rate_processor->setKey($this->ifSet($company_settings['exchange_rates_processor_key']));
            }
