...
class GatewayManager extends AppModel
{
     ...
     * @param bool $include_info True to include gateway info from the source file
     ...
     * @param int $gateway_id The gateway ID to verify is installed and active for the given currency
     * @param int $currency The ISO 4217 currency code to process
     * @param bool $include_info True to include gateway info, false otherwise
     * @return mixed A stdClass object representing the installed gateway,
     *  false if no such gateway exists or is not installed
     */
        ...
        foreach ($files as $file) {
            ...
            if (substr($file, 0, 1) != '.'
                ...
                foreach ($gateway_files as $gateway) {
                    // If the file is not a hidden file, and is a directory, accept it
                    if (substr($gateway, 0, 1) != '.'
                        && is_dir(COMPONENTDIR . 'gateways' . DS . $file . DS . $gateway)
                    ) {
                        try {
    ...
    public function add(array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventListener->register('GatewayManager.add');
            $eventListener->trigger(
                $eventFactory->event('GatewayManager.add', ['gateway_id' => $gateway_id, 'vars' => $vars])
            );
    ...
    public function edit($gateway_id, array $vars)
    {
        ...
        if (!$errors) {
            ...
            if ($this->Input->validates($vars)) {
                ...
                $eventFactory = $this->getFromContainer('util.events');
                $eventListener = $eventFactory->listener();
                $eventListener->register('GatewayManager.edit');
                $eventListener->trigger(
                    $eventFactory->event(
                        'GatewayManager.edit',
    ...
    public function delete($gateway_id)
    {
        ...
        if ($installed_gateway) {
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('GatewayManager.delete');
            $eventListener->trigger(
                $eventFactory->event(
