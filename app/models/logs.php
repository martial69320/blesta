...
class Logs extends AppModel
{
    ...
    public function addCron(array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $fields = ['run_id', 'event', 'group', 'output', 'start_date', 'end_date'];
    ...
    private function getCronLogs()
    {
        ...
        // @deprecated since 4.3 - remove the 'plugin_dir' field set only for backward compatibility
        $deprecated_fields = [
            'IF(cron_tasks.task_type=?,cron_tasks.dir,NULL)' => 'plugin_dir'
        ];

        $this->Record->select($fields)
            ->select($deprecated_fields, false)
            ->appendValues(['plugin'])
            ->from('log_cron')
            ->innerJoin('cron_task_runs', 'cron_task_runs.id', '=', 'log_cron.run_id', false)
            ->innerJoin('cron_tasks', 'cron_tasks.id', '=', 'cron_task_runs.task_id', false);
