...
class ClientGroups extends AppModel
{
    ...
    public function add(array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventListener->register('ClientGroups.add');
            $eventListener->trigger(
                $eventFactory->event('ClientGroups.add', ['client_group_id' => $client_group_id, 'vars' => $vars])
            );
    ...
    public function edit($client_group_id, array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('ClientGroups.edit');
            $eventListener->trigger(
                $eventFactory->event(
                    'ClientGroups.edit',
    ...
    public function delete($client_group_id)
    {
        ...
        $eventFactory = $this->getFromContainer('util.events');
        $eventListener = $eventFactory->listener();
        $eventListener->register('ClientGroups.delete');
        $eventListener->trigger(
            $eventFactory->event(
                'ClientGroups.delete',
