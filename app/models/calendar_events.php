...
class CalendarEvents extends AppModel
{
    ...
    public function add(array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventListener->register('CalendarEvents.add');
            $eventListener->trigger(
                $eventFactory->event('CalendarEvents.add', ['calendar_event_id' => $calendar_event_id, 'vars' => $vars])
            );
    ...
    public function edit($calendar_event_id, array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('CalendarEvents.edit');
            $eventListener->trigger(
                $eventFactory->event(
                    'CalendarEvents.edit',
                    [
    ...
    public function delete($calendar_event_id)
    {
        ...
        $eventFactory = $this->getFromContainer('util.events');
        $eventListener = $eventFactory->listener();
        $eventListener->register('CalendarEvents.delete');
        $eventListener->trigger(
            $eventFactory->event(
    ...
    private function getRules(array $vars, $edit = false)
    {
        $rules = [
            ...
            'start_date' => [
                ...
                'order' => [
                    'rule' => [[$this, 'validateDateOrder'], $this->ifSet($vars['end_date'])],
        ...
        if ($edit) {
            ...
            $rules['staff_id']['valid'] = [
                'rule' => [[$this, 'validateStaffEvent'], $this->ifSet($vars['calendar_event_id'])],
