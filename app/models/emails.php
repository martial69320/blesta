        ...
        $rules = [
            ...
            'company_id' => [
                ...
                'unique' => [
                    'rule' => [
                        [$this, 'validateUnique'],
                        $this->ifSet($vars['email_group_id']),
                        $this->ifSet($vars['lang'])
                    ],
            ...
            'email_signature_id' => [
                'exists' => [
                    //'if_set' => true,
                    'rule' => [[$this, 'validateSignatureExists'], $this->ifSet($vars['company_id'])],
                    'message' => $this->_('Emails.!error.email_signature_id.exists')
                ]
            ],
            'include_attachments' => [
    ...
    private function getSendEmailRules(array $vars, $custom = false)
    {
        ...
        if (!$custom) {
            $rules = [
                'action' => [
                    'exists' => [
                        'rule' => [[$this, 'validateEmailGroupAction'], $this->ifSet($vars['action'])],
        ...
        $rules['to_addresses'] = [
            'format' => [
                'rule' => [[$this, 'validateEmailAddresses'], $this->ifSet($vars['to_addresses'])],
        ...
        if (!empty($vars['cc_addresses'])) {
            $rules['cc_addresses'] = [
                'format' => [
                    'rule' => [[$this, 'validateEmailAddresses'], $this->ifSet($vars['cc_addresses'])],
        ...
        if (!empty($vars['bcc_addresses'])) {
            $rules['bcc_addresses'] = [
                'format' => [
                    'rule' => [[$this, 'validateEmailAddresses'], $this->ifSet($vars['bcc_addresses'])],
        ...
        if (!empty($vars['attachments'])) {
            $rules['attachments'] = [
                'exist' => [
                    'rule' => [[$this, 'validateAttachmentPaths'], $this->ifSet($vars['attachments'])],
