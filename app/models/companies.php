...
class Companies extends AppModel
{
    ...
    public function add(array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            try {
                ...
                $eventListener->register('Companies.add');
                $eventListener->trigger(
                    $eventFactory->event('Companies.add', ['company_id' => $company_id, 'vars' => $vars])
                );
            } catch (Exception $e) {
                $this->rollBack();
                $this->Record->reset();
    ...
    public function edit($company_id, array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('Companies.edit');
            $eventListener->trigger(
                $eventFactory->event(
                    'Companies.edit',
    ...
    public function delete($company_id)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('Companies.delete');
            $eventListener->trigger(
                $eventFactory->event('Companies.delete', ['company_id' => $company_id, 'old_company' => $company])
            );
        }
    }
        ...
        if (is_dir($base_dir)) {
            if ($dh = opendir($base_dir)) {
                while (($directory = readdir($dh)) !== false) {
                    ...
                    $view_dirs[$directory] = $this->Json->decode(
