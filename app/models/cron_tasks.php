...
class CronTasks extends AppModel
{
    ...
    /**
     * Updates a given task to set fields necessary to maintain backward compatibility pre-4.3.0
     * These fields are deprecated as of v4.3.0
     *
     * @deprecated since 4.3.0
     *  - when removing this method, also remove all backward-compatibility updates for 'plugin_dir' support
     *
     ...
     */
    private function makeBackwardCompatible(stdClass $task, $set_plugin_fields = false)
    {
        // The 'dir' field used to be named 'plugin_dir', so set it
        $task->plugin_dir = $task->dir;

        // Tasks returned from ::getTaskRun, ::getTaskRunByKey, and ::getAllTaskRun used to include
        // these additional plugin fields
        if ($set_plugin_fields) {
            $task->plugin_id = (isset($task->plugin->id) ? $task->plugin->id : null);
            $task->plugin_name = (isset($task->plugin->name) ? $task->plugin->name : null);
            $task->plugin_version = (isset($task->plugin->version) ? $task->plugin->version : null);
            $task->plugin_enabled = (isset($task->plugin->enabled) ? $task->plugin->enabled : null);
        }
    ...
    private function setTaskFields(stdClass $task, $set_plugin_fields = false)
    {
        ...
        return $this->makeBackwardCompatible($task, $set_plugin_fields);
    }
    ...
    public function add(array $vars)
    {
        ...
        if (!empty($vars['plugin_dir']) && !array_key_exists('dir', $vars) && !array_key_exists('task_type', $vars)) {
     ...
     *
     * @deprecated since 4.3.0
     * @see CronTasks::deleteTask
     *
     * @param int $task_id The ID of this cron task
     * @param string $plugin_dir The directory this cron task belongs to
     */
    public function delete($task_id, $plugin_dir)
    {
        ...
        $this->Record->from('cron_task_runs')
            ...
            ->where('cron_task_runs.task_id', '=', $task_id)
     ...
     */
    public function getAllTaskRun($system = false, $task_type = 'all')
    {
        $this->Record = $this->getAllTaskRuns();
        ...
        if ($system) {
            ...
            if (Configure::get('Blesta.company_id')) {
                ...
                if (array_key_exists($task_type, $this->getTaskTypes())) {
                    $this->Record->where('cron_tasks.task_type', '=', $task_type);
                }
            }
        }
    ...
    /**
     * Validates whether the given cron task key and directory are in use by another cron task
     *
     * @deprecated since v4.3.0 - Do not use
     *
     ...
     * @return bool True if the given key is unique, false otherwise
     */
     ...
     *
     * @param string $type The type of cron task ('time' or 'interval')
     * @return bool True if the type is valid, false otherwise
     */
    public function validateTaskType($type)
    {
        return in_array($type, ['time', 'interval']);
    }
