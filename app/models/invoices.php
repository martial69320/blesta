...
class Invoices extends AppModel
{
    ...
    public function add(array $vars)
    {
        ...
        do {
            ...
            try {
                ...
                if ($tries > 0 && $e->getCode() == '40001' && strpos($e->getMessage(), '1213') !== false) {
                    $retry = true;
                }
    ...
    private function makeInvoice(array $vars, array $client_settings)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            if (!$this->Input->errors()) {
                ...
                $eventListener->register('Invoices.add');
                $eventListener->trigger($eventFactory->event('Invoices.add', compact('invoice_id')));
    ...
    public function edit($invoice_id, array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('Invoices.edit');
            $eventListener->trigger(
                $eventFactory->event('Invoices.edit', ['invoice_id' => $invoice_id, 'old_invoice' => $invoice])
            );
    ...
    private function lineItemsChanged($invoice_id, $vars)
    {
        ...
        foreach ($new_line_items as $new_line_item) {
            ...
            if (!array_key_exists('id', $new_line_item) || !array_key_exists($new_line_item['id'], $temp_line_items)) {
                return true;
            }
    ...
    private function addLineTax($line_id, $tax_id, $cascade = false)
    {
        $this->Record->insert(
            'invoice_line_taxes',
            ['line_id' => $line_id, 'tax_id' => $tax_id, 'cascade' => ($cascade ? 1 : 0)]
        );
    }
    ...
    public function createFromServices(
        ...
        if (isset($delivery_method->value)
            && array_key_exists($delivery_method->value, $this->getDeliveryMethods($client_id))
        ) {
    ...
    private function getLinesForServices(
        ...
        foreach ($service_ids as $service_id) {
            ...
            foreach ($presenter->discounts() as $discount) {
                ...
                // The total discount is the negated total. It is taxed by default since the Pricing library used by
                // Blesta taxes discounts by default. This can be overriden by using the discount_taxes setting. This
                // setting is not used by Blesta currently, but if it ever is then this tax setting should be based on
                // it as well
                $items[] = [
                    'service_id' => $service_id,
                    'qty' => 1,
    ...
    public function setClosed($invoice_id)
    {
        ...
        if ($invoice) {
            ...
            if ($invoice->paid >= $invoice->total && ($invoice->status == 'active' || $invoice->status == 'proforma')) {
                ...
                $eventFactory = $this->getFromContainer('util.events');
                $eventListener = $eventFactory->listener();
                $eventListener->register('Invoices.setClosed');
                $eventListener->trigger(
                    $eventFactory->event('Invoices.setClosed', ['invoice_id' => $invoice_id, 'old_invoice' => $invoice])
                );
    ...
    {
        $fields = ['id', 'invoice_id', 'service_id', 'description', 'qty', 'amount', 'qty*amount' => 'subtotal'];
    ...
    {
        $fields = ['taxes.*', 'invoice_line_taxes.cascade'];
     ...
     */
    public function getList($client_id = null, $status = 'open', $page = 1, $order_by = ['date_due' => 'ASC'])
    {
     ...
     * @return array An array of stdClass objects containing recurring invoice information
     */
    public function getRecurringList($client_id = null, $page = 1, array $order = ['id' => 'asc'])
    {
    ...
    private function getClientSettingSubquery($setting)
    {
        ...
        $sql1 = $this->Record->select($setting_fields)->
            ...
            where('client_settings.key', '=', $setting)->
            group('clients.id')->
            get();
        $values = $this->Record->values;
        $this->Record->reset();
        ...
        $sql2 = $this->Record->select($setting_fields)->
            ...
            where('client_group_settings.key', '=', $setting)->
            group('clients.id')->
            get();
        $values = $this->Record->values;
        $this->Record->reset();
     ...
     * @return Record The partially constructed query Record object
     */
    private function getInvoices($client_id = null, $status = 'open', array $options = [])
    {
        $fields = ['invoices.*',
            'REPLACE(invoices.id_format, ?, invoices.id_value)' => 'id_code',
            'invoice_delivery.date_sent' => 'delivery_date_sent',
        ...
        if ($status !== 'all' && $status !== 'to_print' && $status !== 'printed') {
    ...
    {
        $this->Record->select(['invoices.id'])->from('invoices')->
            innerJoin('clients', 'clients.id', '=', 'invoices.client_id', false)->
            innerJoin('client_groups', 'client_groups.id', '=', 'clients.client_group_id', false);
    ...
    {
        $this->Record->select(['COUNT(invoices_recur.id)' => 'recur_count'])->
            from('invoices_recur')->
            innerJoin('clients', 'clients.id', '=', 'invoices_recur.client_id', false)->
            innerJoin('client_groups', 'client_groups.id', '=', 'clients.client_group_id', false)->
     ...
     * @param array A numerically indexed array of stdClass objects each
     *  representing a tax rule to apply to this client or client group.
    ...
    public function calcLineTotals($client_id, array $vars, array $tax_rules = null, $client_group_id = null)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            foreach ($vars['lines'] as $i => $line) {
                ...
                if ($client_settings['tax_exempt'] != 'true'
                    && $client_settings['enable_tax'] == 'true'
                    && isset($line['tax'])
                    && $line['tax']
                ) {
                    ...
                    foreach ($tax_totals['tax'] as $level_index => $tax_rule) {
                        ...
                        $tax_rule['amount_formatted'] = $this->CurrencyFormat->format(
                            $tax_rule['amount'],
                            $vars['currency']
                        );
            ...
            $totals = [
                'subtotal' => [
                    'amount' => $subtotal,
                    'amount_formatted' => $this->CurrencyFormat->format($subtotal, $vars['currency'])
                ],
                'total' => [
                    'amount' => $total,
                    'amount_formatted' => $this->CurrencyFormat->format($total, $vars['currency'])
                ],
                'total_w_tax' => [
                    'amount' => $total_w_tax,
                    'amount_formatted' => $this->CurrencyFormat->format($total_w_tax, $vars['currency'])
                ],
                'tax' => $tax
            ];
            ...
            if (isset($vars['amount_paid']) && $vars['amount_paid'] > 0) {
                ...
                $totals['total_due'] = [
                    'amount' => $total_due,
                    'amount_formatted' => $this->CurrencyFormat->format($total_due, $vars['currency'])
                ];
            }
    ...
    private function getRules(array $vars)
    {
        $rules = [
            ...
            'date_due' => [
                ...
                'after_billed' => [
                    'rule' => [[$this, 'validateDateDueAfterDateBilled'], $this->ifSet($vars['date_billed'])],
