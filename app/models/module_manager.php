...
class ModuleManager extends AppModel
{
    ...
    {
        $fields = ['id', 'company_id', 'name', 'class', 'version'];
        $module = $this->Record->select($fields)
            ->from('modules')
            ->where('id', '=', $module_id)
            ->fetch();
    ...
    public function getAll($company_id, $sort_by = 'name', $order = 'asc')
    {
        ...
        $modules = $this->Record->select($fields)
            ->from('modules')
    ...
    public function add(array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventListener->register('ModuleManager.add');
            $eventListener->trigger(
                $eventFactory->event('ModuleManager.add', ['module_id' => $module_id, 'vars' => $vars])
            );
    ...
    public function delete($module_id)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('ModuleManager.delete');
            $eventListener->trigger(
                $eventFactory->event(
