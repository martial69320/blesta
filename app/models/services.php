...
class Services extends AppModel
{
    ...
    {
        // Get all child services
        return $this->getServices(null, $status)
            ->where('services.parent_service_id', '=', $parent_service_id)
            ->numResults();
    }
    ...
    public function getAllRenewablePaid($date)
    {
        ...
        return $this->Record->select(['temp_services.*'])->
            from('invoices')->
    ...
    private function getServices($client_id = null, $status = 'active', $children = true)
    {
        $fields = [
            ...
            'pricings.term', 'pricings.period', 'packages.name',
    ...
    public function getPricingInfo($service_id, $currency_code = null)
    {
        ...
        $fields = [
            ...
            'packages.name', 'packages.id' => 'package_id', 'packages.module_id', 'packages.taxable'
        ];
            ...
            if ($module) {
                ...
                */
                ...
                $fields = [
                    ...
                    'module_row_id', 'coupon_id', 'qty', 'override_price', 'override_currency', 'status',
                ...
                $eventListener->register('Services.add');
                $eventListener->trigger(
                    $eventFactory->event('Services.add', ['service_id' => $service_id, 'vars' => $vars])
                );
     ...
     *  - prorate @deprecated since 4.0.0 - Whether or not to prorate price changes on upgrades by
     *      creating an invoice for the difference between the new price and the existing price
     *      only when updating the pricing_id to another with an equivalent
     *      period (i.e. one-time to one-time or recurring to recurring period)
    ...
    public function edit($service_id, array $vars, $bypass_module = false, $notify = false)
    {
        ...
        $eventFactory = $this->getFromContainer('util.events');
        $eventListener = $eventFactory->listener();
        $eventListener->register('Services.edit');
        $eventListener->trigger(
            $eventFactory->event(
                'Services.edit',
        ...
        if (isset($vars['prorate']) && $vars['prorate'] == 'true') {
            $service_options = (isset($service_options) ? $service_options : []);
            $config_options = (isset($config_options) ? $config_options : []);
    ...
    {
        Loader::loadModels($this, ['Invoices', 'Packages']);
        ...
        $prorate_credits = isset($settings['client_prorate_credits'])
            && $settings['client_prorate_credits'] == 'true';

        // Set options
        $now = date('c');
        $options = ['current_date' => $now, 'taxable' => $taxable];
        ...
        if ($new_service->override_price === null && $new_service->override_currency === null) {
            ...
            // Determine whether the renew date changed
            $renew_date_changed = false;
            if ($old_service->date_renews && $new_service->date_renews) {
                $renew_date_changed = $this->dateChanged(
                    $old_service->date_renews . 'Z',
                    $new_service->date_renews . 'Z'
                );
            }
            ...
            // Prorate any config option changes and renew date changes as it applies to config options
            $option_prorate_amounts = $this->getOptionProrateAmounts(
                $old_service,
                $service_options,
                $config_options,
                $now,
            ...
            $start_from_date = $renew_date_changed && $package_prorate_amount > 0
                ? $old_service->date_renews . 'Z'
                : $now;
            ...
            if ($package_prorate_amount > 0) {
                ...
                $description = Language::_(
                    'Invoices.!line_item.service_prorated_upgrade_description_onetime',
                    true,
                    $old_service->package->name,
                    $new_service->package->name,
                    $new_service->name
                );
                if ($recurring && $renew_date) {
                    $description = Language::_(
                        'Invoices.!line_item.service_prorated_upgrade_description',
                        true,
                        $old_service->package->name,
                        $new_service->package->name,
                        $new_service->name,
                ...
                $line_items[] = $this->makeLineItem(
                    $new_service->id,
                    $description,
                    $new_service->qty,
                    $package_prorate_amount,
                    $taxable
                );
            } else {
            ...
            foreach ($option_prorate_amounts as $option_id => $option_amount) {
                if ($option_amount['amount'] > 0 && $option_amount['qty'] > 0) {
                    ...
                    if ($option_amount['value'] && $option_amount['old_value']) {
                        // Set the description
                        $description = Language::_(
                            'Invoices.!line_item.service_option_prorated_upgrade_description_onetime',
                            true,
                            $option->label,
                            $option_amount['old_value'],
                            $option_amount['value']
                        );
                        if ($recurring && $renew_date) {
                            $description = Language::_(
                                'Invoices.!line_item.service_option_prorated_upgrade_description',
                                true,
                                $option->label,
                                $option_amount['old_value'],
                                $option_amount['value'],
                        ...
                        if ($option->type == 'quantity' && isset($option_amount['old_qty'])) {
                            // Set the description
                            $description = Language::_(
                                'Invoices.!line_item.service_option_prorated_upgrade_qty_description_onetime',
                                true,
                                $option->label,
                                $option_amount['old_qty'],
                                $option_amount['old_value'],
                                $option_amount['qty'],
                                $option_amount['value']
                            );
                            if ($recurring && $renew_date) {
                                $description = Language::_(
                                    'Invoices.!line_item.service_option_prorated_upgrade_qty_description',
                                    true,
                                    $option->label,
                                    $option_amount['old_qty'],
                                    $option_amount['old_value'],
                        ...
                        // Set the description
                        $description = Language::_(
                            'Invoices.!line_item.service_option_prorated_addition_description_onetime',
                            true,
                            $option->label,
                            $option_amount['value']
                        );
                        if ($recurring && $renew_date) {
                            $description = Language::_(
                                'Invoices.!line_item.service_option_prorated_addition_description',
                                true,
                                $option->label,
                                $option_amount['value'],
                                $start_date,
                        ...
                        if ($option->type == 'quantity') {
                            // Set the description
                            $description = Language::_(
                                'Invoices.!line_item.service_option_prorated_addition_qty_description_onetime',
                                true,
                                $option->label,
                                $option_amount['qty'],
                                $option_amount['value']
                            ...
                            if ($recurring && $renew_date) {
                                $description = Language::_(
                                    'Invoices.!line_item.service_option_prorated_addition_qty_description',
                                    true,
                                    $option->label,
                                    $option_amount['qty'],
                                    $option_amount['value'],
                    ...
                    } elseif (in_array($option->type, ['text', 'textarea', 'password'])) {
                        // Set the description
                        $description = Language::_(
                            'Invoices.!line_item.service_option_prorated_upgrade_text_description_onetime',
                            true,
                            $option->label
                        );
                        if ($recurring && $renew_date) {
                            $description = Language::_(
                                'Invoices.!line_item.service_option_prorated_upgrade_text_description',
                                true,
                                $option->label,
                                $start_date,
                                $renew_date
                        ...
                        if (isset($option_amount['old_qty'])) {
                            // Set the description
                            $description = Language::_(
                                'Invoices.!line_item.service_option_prorated_addition_text_description_onetime',
                                true,
                                $option->label
                            );
                            if ($recurring && $renew_date) {
                                $description = Language::_(
                                    'Invoices.!line_item.service_option_prorated_addition_text_description',
                                    true,
                                    $option->label,
                                    $start_date,
                                    $renew_date
                    ...
                    if ($option_amount['setup_fee'] > 0) {
                        $description = Language::_(
                            'Invoices.!line_item.service_option_setup_fee_description',
                            true,
                            $option->label,
                            $option_amount['value']
                        );
                        $line_items[] = $this->makeLineItem(
                            ...
                            $description,
                            1,
                            $option_amount['setup_fee'],
                            ($taxable && $setup_fee_taxable)
                        );
        ...
        if ($total > 0) {
            ...
            if ($credits > 0) {
                $line_items[] = $this->makeLineItem(
                    null,
                    Language::_('Invoices.!line_item.prorated_credit', true),
                    1,
                    -$credits,
                    false
                );
    ...
    private function getCouponApplies($old_service, $new_service)
    {
        ...
        if ($old_service->coupon_id) {
            $packages = [$new_service->package->id => $new_service->pricing_id];
     ...
     * @param string $currency The ISO 4217 currency code
     * @param string $date_billed The invoice date billed
     ...
     * @param bool $taxable True if the line item is taxable, or false otherwise
    ...
    private function getRenewDateProrateAmount($old_service, $new_service)
    {
        ...
        $renew_date_changed = false;
        if ($old_service->date_renews && $new_service->date_renews) {
            $renew_date_changed = $this->dateChanged($old_service->date_renews . 'Z', $new_service->date_renews . 'Z');
        }
        ...
        if ($renew_date_changed) {
            ...
            $renew_date_amount = $this->Packages->getProratePrice(
                isset($new_service->package_pricing->price_renews)
                    ? $new_service->package_pricing->price_renews
                    : $new_service->package_pricing->price,
                $old_service->date_renews . 'Z',
                $new_service->package_pricing->term,
                $new_service->package_pricing->period,
                null,
                true,
                $new_service->date_renews . 'Z'
            );
    ...
    private function getPackageProrateAmount($old_service, $new_service, $date)
    {
        ...
        // the service must have either had a renew date in the future, or no
        // renew date at all (i.e. one-time, or recurs in future)
        $period_upgradable = (
            $new_service->package_pricing->period == $old_service->package_pricing->period
            || (
                $new_service->package_pricing->period != 'onetime'
                && $old_service->package_pricing->period != 'onetime'
            )
        );
        $future_renew_date = $new_service->date_renews === null
            || strtotime($new_service->date_renews) > strtotime($this->dateToUtc($date));
        if ($new_service->pricing_id == $old_service->pricing_id
            || $new_service->package->id == $old_service->package->id
            || !$period_upgradable || !$future_renew_date
        ) {
            return 0.0;
        }

        // Get the old and new prices
        $old_price = isset($old_service->package_pricing->price_renews)
            ? $old_service->package_pricing->price_renews
            : $old_service->package_pricing->price;
        $new_price = isset($new_service->package_pricing->price_renews)
            ? $new_service->package_pricing->price_renews
            : $new_service->package_pricing->price;

        // The one-time prorate amount is simply the price difference
        $prorate_amount = $new_price - $old_price;
        ...
        if ($old_service->date_renews !== null && $old_service->package_pricing->period != 'onetime' &&
            ...
            // The total prorate amount is the difference between the prorated
            // amount being added and the amount being removed
            $service_prorate_amount = $this->Packages->getProratePrice(
                $old_price,
                $date,
                $old_service->package_pricing->term,
                $old_service->package_pricing->period,
                null,
                true,
                $prorate_date
            );
            $new_service_prorate_amount = $this->Packages->getProratePrice(
                $new_price,
                $date,
                $new_service->package_pricing->term,
                $new_service->package_pricing->period,
     ...
     */
    private function getOptionProrateAmounts(
        $old_service,
        ...
        $service_options['add'] = (isset($service_options['add']) ? $service_options['add'] : []);
        foreach ($service_options['add'] as $option_id => $option_pricing_id) {
            ...
            if (array_key_exists($option_id, $config_options)
                ...
                if ($quantity > 0) {
                    // The prorate amount is the full price for one-time
                    // services, or prorated to the renew date otherwise
                    $amount = $pricing->price;
                    if ($pricing->period != 'onetime' && $prorate_date) {
                        $amount = $this->Packages->getProratePrice(
                            $pricing->price,
                            $date,
                            $pricing->term,
                            $pricing->period,
        ...
        $service_options['edit'] = (isset($service_options['edit']) ? $service_options['edit'] : []);
        foreach ($service_options['edit'] as $option_id => $temp_pricing) {
            ...
            if (array_key_exists($option_id, $config_options) && array_key_exists($option_id, $old_options) &&
                ...
                if ($quantity > 0 && $pricing->period != 'onetime' && $prorate_date) {
                    $new_amount = $this->Packages->getProratePrice(
                        $new_amount,
                        $date,
                        $pricing->term,
                        $pricing->period,
                ...
                if ($old_quantity > 0
                    && $old_options[$option_id]->option_pricing_period != 'onetime'
                    && $prorate_date
                ) {
                    $old_amount = $this->Packages->getProratePrice(
                        $old_amount,
                        $date,
                        $old_options[$option_id]->option_pricing_term,
                        $old_options[$option_id]->option_pricing_period,
                        null,
                        true,
                        $old_service->date_renews
                    );
                }
        ...
        $service_options['delete'] = (isset($service_options['delete']) ? $service_options['delete'] : []);
        foreach ($service_options['delete'] as $option_id => $option_pricing_id) {
            // Verify the value and pricing exist, as expected
            if (array_key_exists($option_id, $old_options)) {
                // The prorate amount is the full price for one-time services,
                // or prorated to the previous renew date otherwise
    ...
    /**
     * Determines whether the new date and old date differ by a day of the month
     *
     * @param string $old_date The old date in UTC
     * @param string $new_date The new date in UTC
     * @return bool True if the date has changed by a day of the month, or false otherwise
     */
    private function dateChanged($old_date, $new_date)
    {
        $Date = clone $this->Date;
        $Date->setTimezone('UTC', Configure::get('Blesta.company_timezone'));

        return ($Date->cast($old_date, 'Ymd') != $Date->cast($new_date, 'Ymd'));
    }
    ...
    public function delete($service_id, $validate = true)
    {
        ...
        if ($validate) {
            ...
            $rules = [
                ...
                'status' => [
                    'valid' => [
                        'rule' => ['in_array', ['pending', 'in_review']],
    ...
    public function cancel($service_id, array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('Services.cancel');
            $eventListener->trigger(
                $eventFactory->event(
                    'Services.cancel',
                    ['service_id' => $service_id, 'vars' => $vars, 'old_service' => $service]
            ...
            if (isset($vars['status']) && $vars['status'] == 'canceled') {
                ...
                $void_invoice_setting = $this->SettingsCollection->fetchClientSetting(
                    $service->client_id,
                    $this->ifSet($this->Clients, null),
                    'void_invoice_canceled_service'
                );

                if ($this->ifSet($void_invoice_setting['value']) == 'true') {
                    ...
                    $options['void_inv_canceled_service_days'] = $this->ifSet(
    ...
    private function voidInvoices($service_id, array $options = [])
    {
        ...
        $void_inv_canceled_service_days = $this->ifSet($options['void_inv_canceled_service_days'], '0');
        ...
        foreach ($invoices as $invoice) {
            ...
            $notes = $this->ifSet($invoice->note_private);
            $vars = [
    ...
    public function unCancel($service_id)
    {
        ...
        $this->Record->where('services.id', '=', $service_id)->
            ...
            update('services', ['date_canceled' => null]);
    ...
    public function suspend($service_id, array $vars = [])
    {
        ...
        $eventFactory = $this->getFromContainer('util.events');
        $eventListener = $eventFactory->listener();
        $eventListener->register('Services.suspend');
        $eventListener->trigger(
            $eventFactory->event(
                'Services.suspend',
                ['service_id' => $service_id, 'vars' => $vars, 'old_service' => $service]
    ...
    public function unsuspend($service_id, array $vars = [])
    {
        ...
        $eventFactory = $this->getFromContainer('util.events');
        $eventListener = $eventFactory->listener();
        $eventListener->register('Services.unsuspend');
        $eventListener->trigger(
            $eventFactory->event(
                'Services.unsuspend',
                ['service_id' => $service_id, 'vars' => $vars, 'old_service' => $service]
    ...
    public function renew($service_id)
    {
        ...
        if ($module_data) {
            ...
            if ($module) {
                ...
                // Delete the association used for renewal between the service and any existing invoices
                $this->ServiceInvoices->delete($service_id);
    ...
    public function getOptions($service_id)
    {
        $fields = ['service_options.*', 'package_option_values.value' => 'option_value',
            ...
            'pricings.setup_fee' => 'option_pricing_setup_fee', 'pricings.currency' => 'option_pricing_currency'
        ];
    ...
    {
        Loader::loadModels($this, ['Companies', 'Coupons', 'Invoices']);
        Loader::loadComponents($this, ['SettingsCollection']);
    ...
    public function getDataPresenter($client_id, array $vars, array $options)
    {
        Loader::loadModels(
            $this,
            ['Companies', 'Clients', 'ClientGroups', 'Coupons', 'Invoices', 'Packages', 'PackageOptions']
        );
        Loader::loadComponents($this, ['SettingsCollection']);
     ...
     *
     * @deprecated since 4.0.0 - Use Services::getDataPresenter
     *
    ...
    {
        Loader::loadModels($this, ['Clients', 'Coupons', 'Invoices', 'Packages', 'PackageOptions']);

        $list = ['items' => [], 'discounts' => [], 'taxes' => []];
        ...
        Loader::loadComponents($this, ['PricingPresenter' => [$client->settings, $tax_rules, $coupons]]);
