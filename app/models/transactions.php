...
class Transactions extends AppModel
{
    ...
    public function add(array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $fields = [
                ...
                'account_id', 'gateway_id', 'transaction_id',
                'parent_transaction_id', 'reference_id', 'status', 'date_added'
            ];
            ...
            $eventListener->register('Transactions.add');
            $eventListener->trigger($eventFactory->event('Transactions.add', compact('transaction_id')));
    ...
    public function edit($transaction_id, array $vars, $staff_id = null)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $fields = [
                ...
                'account_id', 'gateway_id', 'transaction_id',
                'parent_transaction_id', 'reference_id', 'status'
            ];
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('Transactions.edit');
     ...
     */
    public function getList($client_id = null, $status = 'approved', $page = 1, $order_by = ['date_added' => 'DESC'])
    {
    ...
    public function getCredits($client_id, $currency = null)
    {
        ...
        foreach ($transactions as $transaction) {
            ...
            if (!array_key_exists($transaction->currency, $precisions)) {
    ...
    {
        $this->Record->select(['COUNT(*)' => 'status_count'])->from('transactions')->
            innerJoin('clients', 'clients.id', '=', 'transactions.client_id', false)->
            innerJoin('client_groups', 'client_groups.id', '=', 'clients.client_group_id', false)->
    ...
    public function verifyApply(array &$vars, $validate_trans_id = true, $total = null)
    {
        ...
        $rules = [
            'transaction_id' => [
                ...
                'currency_matches' => [
                    'rule' => [[$this, 'validateCurrencyAmounts'], (array) $this->ifSet($vars['amounts'], [])],
    ...
    {
        $currency = $this->ifSet($vars['currency']);
