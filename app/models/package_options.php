...
class PackageOptions extends AppModel
{
    ...
    public function getByPackageId($package_id)
    {
        ...
        $options = $this->Record->select($fields)
            ...
            ->order(['package_option_groups.name' => 'ASC', 'package_option_group.order' => 'ASC'])
            ->fetchAll();
        ...
        if (isset($vars['values']) && is_array($vars['values'])) {
            foreach ($vars['values'] as $value) {
                ...
                if (isset($vars['type']) && $vars['type'] != 'quantity' && array_key_exists('default', $temp_val)) {
                    $temp_val['default'] = (int)($this->ifSet($temp_val['default'], '0') == '1');
                }
    ...
    public function add(array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $fields = ['company_id', 'label', 'name', 'description', 'type', 'addable', 'editable'];
    ...
    public function edit($option_id, array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            // Add the package option
            $fields = ['label', 'name', 'description', 'type', 'addable', 'editable'];
    ...
    public function delete($option_id)
    {
        ...
        $this->Record->from('package_options')
            ...
            ->delete([
                ...
                'package_option_values.*', 'package_option_pricing.*', 'pricings.*'
            ]);
    }
    ...
    private function addOptionValues($option_id, $company_id, array $values, $type)
    {
        ...
        for ($i = 0; $i < $num_values; $i++) {
            ...
            if ($this->canDeleteValue($this->ifSet($values[$i]['id']), $this->ifSet($values[$i]['name']), $type)) {
            ...
            if ($this->canEditValue($this->ifSet($values[$i]['id']))) {
                ...
                case 'checkbox':
                    ...
                    if ($pricing->setup_fee > 0) {
                        $field_label_name = Language::_(
                            ...
                            true,
                            $value->name,
                            $this->CurrencyFormat->format($price, $pricing->currency),
                            $this->CurrencyFormat->format($pricing->setup_fee, $pricing->currency),
    ...
    private function getRules(array $vars, $edit = false)
    {
        ...
        $rules = [
            ...
            'values' => [
                    ...
                    'rule' => [
                        function($values, $type) {
                            ...
                            foreach ($values as $value) {
                                ...
                                $id = $this->ifSet($value['id']);
                                $name = $this->ifSet($value['name']);
                                if ($this->canDeleteValue($id, $name, $type)) {
                                    continue;
                                }

                                if ($this->ifSet($value['status'], 'active') == 'active') {
                                    return true;
                                }
                            }
                ...
                'single_default_value' => [
                    'rule' => function($values) {
                        ...
                        $total_defaults = 0;
                        foreach ($values as $value) {
                            if ($this->ifSet($value['default'], '0') == '1') {
                                $total_defaults++;
                            }
                        }
                ...
                'qty_valid' => [
                    ...
                    'rule' => [
                        function ($default, $type, $min, $max, $step) {
                            ...
                            if ($type != 'quantity' || !is_numeric($default)) {
                                return true;
                            }
            ...
            'groups' => [
                'exists' => [
                    'if_set' => true,
                    'rule' => [[$this, 'validateGroupIds'], $this->ifSet($vars['company_id'], null)],
