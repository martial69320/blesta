...
class Clients extends AppModel
{
     ...
     *
     * @param $vars An array of client info including:
     *
    ...
    {
        Loader::loadModels($this, ['Users', 'Contacts', 'Companies']);
        ...
        if (!empty($errors)) {
            ...
            $eventListener->register('Clients.create');
            $eventListener->trigger($eventFactory->event('Clients.create', ['client' => $client]));
    ...
    public function add(array $vars)
    {
         ...
         */
        $values = [$company_settings['clients_start'], $company_settings['clients_increment'],
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('Clients.add');
            $eventListener->trigger($eventFactory->event('Clients.add', ['client_id' => $client_id, 'vars' => $vars]));
    ...
    public function delete($client_id)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('Clients.delete');
            $eventListener->trigger(
                $eventFactory->event('Clients.delete', ['client_id' => $client_id, 'old_client' => $client])
            );
        }
    }
    ...
    public function addNote($client_id, $staff_id, array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('Clients.addNote');
            $eventListener->trigger(
                $eventFactory->event('Clients.addNote', ['note_id' => $note_id, 'vars' => $vars])
            );
    ...
    public function editNote($note_id, array $vars)
    {
        ...
        if ($this->Input->validates($vars)) {
            ...
            $eventFactory = $this->getFromContainer('util.events');
            $eventListener = $eventFactory->listener();
            $eventListener->register('Clients.editNote');
            $eventListener->trigger(
                $eventFactory->event('Clients.editNote', ['note_id' => $note_id, 'vars' => $vars, 'old_note' => $note])
            );
        }
    }
    ...
    public function deleteNote($note_id)
    {
        ...
        $eventFactory = $this->getFromContainer('util.events');
        $eventListener = $eventFactory->listener();
        $eventListener->register('Clients.deleteNote');
        $eventListener->trigger(
            $eventFactory->event('Clients.deleteNote', ['note_id' => $note_id, 'old_note' => $note])
        );
    }
     ...
     */
    public function getList($status = null, $page = 1, $order_by = ['id_code' => 'ASC'])
    {
    ...
    private function searchClients($query)
    {
        ...
        $this->Record->select(['temp.*'])->appendValues($values)->from([$sub_query_sql => 'temp'])->
     ...
     */
    public function getMailLogList($client_id, $page = 1, $order_by = ['date_sent' => 'DESC'])
    {
        $this->Record = $this->getMailLog($client_id);
    ...
    private function getAddRules($vars, $edit = false)
    {
        ...
        if ($edit) {
            ...
            $rules['user_id']['unique']['rule'] = [[$this, 'validateUserId'], $this->ifSet($vars['client_id'])];
    ...
    private function getCustomFieldRules(array $vars)
    {
        $rules = [
            ...
            'values' => [
                'format' => [
                    'if_set' => true,
                    'rule' => [[$this, 'validateValues'], $this->ifSet($vars['type'])],
