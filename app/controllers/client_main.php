...
class ClientMain extends ClientController
{
    ...
    private function addCustomFields(array $vars = [])
    {
        ...
        foreach ($custom_field_ids as $field) {
            if (!isset($custom_fields_set[$field->id])) {
                ...
                if ($field->read_only != '1' && $field->show_client == '1') {
    ...
    public function setLanguage()
    {
        ...
        if (($client = $this->Clients->get($this->Session->read('blesta_client_id')))
            && isset($client->settings['client_set_lang'])
            && $client->settings['client_set_lang'] == 'true'
        ) {
            // Set the language for this session if the client is allowed
            $this->Session->write('blesta_language', $language->code);

            // If this is the primary user for this client set the client's language preference
            if ($client->user_id == $this->Session->read('blesta_id')) {
                $this->Clients->setSetting($client->id, 'language', $language->code);
            }
        } elseif (($show_lang = $this->Companies->getSetting(Configure::get('Blesta.company_id'), 'client_set_lang'))
            && $show_lang->value == 'true'
        ) {
