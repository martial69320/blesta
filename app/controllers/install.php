...
class Install extends Controller
{
    ...
    private function systemRequirementsCli()
    {
        ...
        $hotfix = $this->setHotfixMessage(true);

        if (!empty($hotfix)) {
            $this->Console->output($hotfix . "\n");
        }
    ...
    /**
     * Sets a notice message regarding the required hotfix files being used if the ioncube loader is too old
     *
     * @param bool $return Whether or not to return the message
     *  rather than setting it to the view (optional, default false)
     * @return void|string The message to return if $return is true, otherwise void
     */
    private function setHotfixMessage($return = false)
    {
        Language::loadLang('system_requirements');

        $message = '';

        // Hotfix files must be used if the ioncube loader does not support the bundle
        if (extension_loaded('ioncube loader') && function_exists('ioncube_loader_version')) {
            // Minimum v10.1.0 to use the bundled version
            if (version_compare(ioncube_loader_version(), '10.1', '<')) {
                $hotfix_versions = [
                    '5.4.0' => DS . 'hotfix-php54' . DS,
                    '5.6.0' => DS . 'hotfix-php7' . DS,
                    '7.1.0' => DS . 'hotfix-php71' . DS
                ];
                foreach ($hotfix_versions as $version => $directory) {
                    if (version_compare(PHP_VERSION, $version, '>=')) {
                        $message = Language::_('SystemRequirements.!info.ext-ioncube.minimum', true, $directory);
                    }
                }
            }
        }

        // Return the message if set to
        if ($return) {
            return $message;
        }
    ...
    protected function getMinReq()
    {
        ...
        $reqs = [
            'php' => [
                'message' => Language::_('SystemRequirements.!error.php.minimum', true, '5.4.0', PHP_VERSION),
                'req' => true,
                'cur' => version_compare(PHP_VERSION, '5.4.0', '>=')
            ],
            'ext-pdo' => [
    ...
    protected function getRecReq()
    {
        ...
        $reqs = [
            ...
            'ext-mcrypt' => [
                'message' => Language::_('SystemRequirements.!warning.mcrypt.recommended', true),
                'req' => true,
                'cur' => extension_loaded('mcrypt')
            ], // Faster encryption/decryption
            'cache_writable' => [
