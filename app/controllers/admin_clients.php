...
class AdminClients extends AppController
{
    ...
    public function index()
    {
        ...
        $clients = $this->Clients->getList($status, $page, [$sort => $order]);
    ...
    public function view($content = null)
    {
        ...
        $this->uses(['ClientGroups', 'Contacts', 'Invoices', 'Logs', 'PluginManager']);
    ...
    public function services()
    {
        ...
        $services = $this->Services->getList($client->id, $status, $page, [$sort => $order], false);
    ...
    public function invoices()
    {
        ...
        if ($status == 'recurring') {
            ...
            $invoices = $this->Invoices->getRecurringList($client->id, $page, [$sort => $order]);
            $total_results = $this->Invoices->getRecurringListCount($client->id);
        } else {
            $sort = (isset($this->get['sort']) ? $this->get['sort'] : 'date_due');
            $order = (isset($this->get['order']) ? $this->get['order'] : 'desc');

            $invoices = $this->Invoices->getList($client->id, $status, $page, [$sort => $order]);
    ...
    public function transactions()
    {
        ...
        $this->set('transactions', $this->Transactions->getList($client->id, $status, $page, [$sort => $order]));
        $this->set('client', $client);
        $this->set('status', $status);
        $this->set('status_count', $status_count);
        $this->set('sort', $sort);
    ...
    {
        $this->uses(
            ['ClientGroups', 'Companies', 'Contacts', 'Countries', 'Currencies', 'Languages', 'States', 'Users']
        );
        $this->components(['SettingsCollection']);
    ...
    {
        $this->uses(['ClientGroups', 'Contacts', 'Countries', 'Currencies', 'Languages', 'States', 'Users']);
        $this->components(['SettingsCollection']);
        $this->ArrayHelper = $this->DataStructure->create('Array');
    ...
    public function email()
    {
        ...
        if (!empty($this->post)) {
            ...
            $this->Emails->sendCustom(
                $this->Html->ifSet($this->post['from']),
                $this->Html->ifSet($this->post['from_name']),
                $this->Html->ifSet($this->post['to']),
                $this->Html->ifSet($this->post['subject']),
                ['html' => $this->Html->ifSet($this->post['html']), 'text' => $this->Html->ifSet($this->post['text'])],
                null,
                null,
                null,
                null,
        ...
        // Include WYSIWYG
        $this->Javascript->setFile('blesta/ckeditor/ckeditor.js', 'head', VENDORWEBDIR);
        $this->Javascript->setFile('blesta/ckeditor/adapters/jquery.js', 'head', VENDORWEBDIR);
    ...
    private function getCcAccountInfo($vars)
    {
        ...
        $account_info['card_number'] = isset($vars->number)
            ? $vars->number
            : null;
        $account_info['card_exp'] = isset($vars->expiration_year) && isset($vars->expiration_month)
            ? $vars->expiration_year . $vars->expiration_month
            : null;
        $account_info['card_security_code'] = isset($vars->security_code)
            ? $vars->security_code
            : null;
        $account_info['reference_id'] = isset($vars->reference_id)
            ? $vars->reference_id
            : null;
        $account_info['client_reference_id'] = isset($vars->client_reference_id)
            ? $vars->client_reference_id
            : null;
    ...
    private function setCcView(stdClass $vars, stdClass $client, $edit = false, $save_account = false)
    {
        ...
        $gateway_form = $this->Payments->getBuildCcForm($client->settings['default_currency']);
    ...
    public function packages()
    {
        ...
        $this->set('vars', $vars);
        $this->set('packages', $this->Packages->getAll($this->company_id, ['name' => 'ASC'], 'restricted'));
    ...
    public function editService()
    {
        $this->uses(
            ['Currencies', 'Invoices', 'Services', 'ServiceChanges', 'Packages',
                'PackageOptions', 'ModuleManager', 'Coupons']
        );
                ...
                switch ($this->post['section']) {
                    ...
                    case 'package':
                        ...
                        if (empty($errors) && $prorate && $total > 0) {
                            $invoice_data = $this->makeInvoice($client, $serviceChange, $pricing->currency, true);
    ...
    public function editServiceAddons()
    {
        ...
        if ($service->package_group_id) {
            list($package_options, $package_attributes) = $this->listPackages(
                $client->id,
    ...
    private function getServiceTabs(stdClass $service, stdClass $package, $module, $method = null, $plugin_id = null)
    {
        ...
        if ($service->status != 'canceled') {
            ...
            $module_tabs = ($module ? $module->getAdminTabs($package) : []);
     ...
     */
    private function makeInvoice(stdClass $client, PresenterInterface $presenter, $currency, $deliver = true)
    {
        ...
        $invoice_vars = [
            ...
            'date_due' => date('c'),
            'currency' => $currency,
            'lines' => $this->makeLineItems($presenter)
        ];
     ...
     */
    private function makeLineItems(PresenterInterface $presenter)
    {
        $items = [];
