...
class WhmcsMigrator extends Migrator
{
     ...
     */
    private $default_client_group_name = 'unknown';
    /**
    ...
    protected function importClients()
    {
        ...
        foreach ($clients as $client) {
            ...
            $vars = [
                ...
                'state' => $this->decode($client->state != '' ? substr($client->state, 0, 3) : null),
    ...
    protected function importServices()
    {
        ...
        foreach ($services as $service) {
            ...
            $pricing = $this->getPricing(
                $this->WhmcsServices->getTerm($service->registrationperiod),
                $package,
                $currency,
            ...
            $override_price = (
                ...
                == number_format($service->recurringamount, 2, '.', '')
                ? $p
                : null
            );
            $override_currency = ($override_price === null ? null : $currency);
            ...
            $vars = [
                ...
                'override_currency' => $override_currency,
                'status' => $status,
                'date_added' => $this->getValidDate($service->registrationdate),
                'date_renews' => $this->getValidDate($service->nextinvoicedate, 'Y-m-d H:i:s', true),
                'date_last_renewed' => null,
                'date_suspended' => $status == 'suspended' ? $this->Companies->dateToUtc(date('c')) : null,
                'date_canceled' => $status == 'canceled' ? $this->Companies->dateToUtc(date('c')) : null
    ...
    protected function importSupportDepartments()
    {
        ...
        foreach ($departments as $department) {
            $vars = [
                ...
                'pass' => $this->decode($this->decryptData($department->password)),
    ...
    protected function importSupportTickets()
    {
        ...
        foreach ($tickets as $ticket) {
            $vars = [
                ...
                'department_id' => isset($this->mappings['support_departments'][$ticket->did])
                    ? $this->mappings['support_departments'][$ticket->did]
                    : 0,
                'staff_id' => null,
                'service_id' => null,
                ...
                'date_updated' => $this->getValidDate($ticket->date),
                'date_closed' => isset($statuses[$ticket->status])
    ...
    protected function importMisc()
    {
        ...
        foreach ($this->WhmcsEmails->get() as $message) {
            ...
            $vars = [
                ...
                'sent' => 1,
                'error' => null,
                'date_sent' => $this->getValidDate($message->date)
            ];
     ...
     */
    private function getValidDate($date, $format = 'Y-m-d H:i:s', $error_null = false)
    {
