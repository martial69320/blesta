...
class OrderFormController extends OrderController
{
    ...
    public function preAction()
    {
        ...
        Language::loadLang(
            [Loader::fromCamelCase(get_class($this)), $this->order_form->type, 'main', 'summary'],
            null,
            PLUGINDIR . DS . 'order' . DS . 'views' . DS . 'templates'
            . DS . $this->order_form->template . DS . 'language' . DS
        );
    ...
    private function getCartItemsByPeriod($items)
    {
        ...
        foreach ($items as $item) {
            ...
            if (!array_key_exists($key, $grouped_items)) {
    ...
    private function getLineTotals($client_id, &$cart, $recurring = false)
    {
        ...
        $line_totals = ['total' => 0, 'subtotal' => 0, 'tax' => []];
        $cart['display_items'] = [];
        ...
        foreach ($valid_groups as $group_id) {
            if ($item_group->type == 'addon') {
                ...
                if ($this->client) {
                    ...
                    $restricted_packages = $this->Packages->getAllPackagesByGroup($group_id, 'restricted');
    ...
    protected function runAntifraudCheck(array $order_settings, stdClass $client)
    {
        ...
        try {
            ...
            $status = $fraud_detect->verify([
                'ip' => $this->Html->ifSet($requestor->ip_address),
