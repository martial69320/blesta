...
class Checkout extends OrderFormController
{
    ...
    {
        $this->uses(['Accounts', 'Contacts', 'Transactions', 'Payments', 'Invoices', 'Order.OrderOrders']);
        $vars = new stdClass();
        ...
        if (!$order && $summary['totals']['total']['amount'] <= 0) {
            // Create the order
    ...
    private function createOrder(array $items, $currency)
    {
        ...
        $details = [
            ...
            'fraud_report' => $this->SessionCart->getData('fraud_report')
                ? $this->Json->encode($this->SessionCart->getData('fraud_report'))
                : null,
            'fraud_status' => $this->SessionCart->getData('fraud_status'),
            'status' => ($this->order_form->manual_review
                || $this->SessionCart->getData('fraud_status') == 'review' ? 'pending' : 'accepted'
            ),
            ...
            if ($pay_with == 'account') {
                ...
                if (isset($this->post['save_details']) && $this->post['save_details'] == 'true') {
                    ...
                    if ($type == 'ach') {
                        ...
                        $account_info['card_number'] = isset($account_info['number']) ? $account_info['number'] : null;
                        $account_info['card_exp'] = (
                                isset($account_info['expiration_year']) && isset($account_info['expiration_month'])
                            )
                            ? $account_info['expiration_year'] . $account_info['expiration_month']
                            : null;
                        $account_info['card_security_code'] = isset($account_info['security_code'])
                            ? $account_info['security_code']
                            : null;
                    }
                }
            }
    ...
    private function authorizePayment(array $vars, array $apply_amounts, $total)
    {
        ...
        if ((isset($vars['payment_account']) && substr($vars['payment_account'], 0, 2) == 'cc')
            ...
            $account_id = null;
            $account_info = null;
            $type = isset($vars['payment_type']) ? $vars['payment_type'] : 'cc';
    ...
    private function getCcAccountInfo($vars)
    {
        ...
        $account['card_number'] = isset($vars['number']) ? $vars['number'] : null;
        $account['card_exp'] = (isset($vars['expiration_year']) && isset($vars['expiration_month']))
            ? $vars['expiration_year'] . $vars['expiration_month']
            : null;
        $account['card_security_code'] = isset($vars['security_code']) ? $vars['security_code'] : null;
        $account['reference_id'] = isset($vars['reference_id']) ? $vars['reference_id'] : null;
        $account['client_reference_id'] = isset($vars['client_reference_id']) ? $vars['client_reference_id'] : null;
    ...
    {
        $this->uses(['Contacts', 'Countries', 'States']);
