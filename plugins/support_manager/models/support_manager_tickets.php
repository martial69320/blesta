...
class SupportManagerTickets extends SupportManagerModel
{
    ...
    public function delete(array $ticket_ids)
    {
        if (!empty($ticket_ids)) {
            $this->Record->from('support_tickets')
                ...
                ->where('support_tickets.id', 'in', $ticket_ids)
                ->delete(['support_tickets.*','support_replies.*','support_attachments.*']);
        }
    }
            ...
            $tags = [
                'SupportManager.ticket_updated' => [
                    'update_ticket_url' => $this->Html->safe(
                        $hostname . $this->getWebDirectory() . Configure::get('Route.client')
    ...
    public function sendEmail($reply_id, $additional_tags = [])
    {
        ...
        if ($ticket && $ticket->reply_type == 'reply') {
            ...
            if ($num_attachments > 0) {
                $num_reply_attachments = $this->Record->select()
    ...
    private function sendTicketByClientEmail($ticket, $new_ticket, $additional_tags = [])
    {
        Loader::loadModels(
            $this,
            ['Emails', 'Staff', 'SupportManager.SupportManagerStaff', 'SupportManager.SupportManagerDepartments']
        );
    ...
    private function getReplyRules(array $vars, $new_ticket = false)
    {
        ...
        if ($new_ticket) {
            ...
            $rules['client_id'] = [
                'attached_to' => [
                    'if_set' => true,
                    'rule' => [[$this, 'validateClientTicket'], $this->ifSet($vars['ticket_id'])],
    ...
    private function getRules(array $vars, $edit = false, $require_email = false)
    {
        $rules = [
            ...
            'service_id' => [
                ...
                'belongs' => [
                    'if_set' => true,
                    'rule' => [[$this, 'validateClientService'], $this->ifSet($vars['client_id'])],
        ...
        if ($edit) {
            ...
            $rules['client_id']['set'] = [
                'rule' => [[$this, 'validateTicketUnassigned'], $this->ifSet($vars['ticket_id'])],
            ...
            $rules['status']['trash'] = [
                'rule' => function ($status) use ($vars) {
                    $ticket = $this->get($this->ifSet($vars['ticket_id']), false);
