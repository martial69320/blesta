...
class SupportManagerStaff extends SupportManagerModel
{
     ...
     */
    private $serializable_settings = ['mobile_ticket_emails', 'ticket_emails'];
    ...
    private function updateStaff($staff_id, $company_id, array $vars)
    {
        ...
        $this->addToDepartments($staff_id, $company_id, $this->ifSet($vars['departments'], []));

        // Add the staff schedules
        $this->updateSchedule($staff_id, $company_id, $this->ifSet($vars['schedules'], []));

        // Add the staff settings
        $this->setSettings($staff_id, $company_id, $this->ifSet($vars['settings'], []));
    ...
    private function getRules(array $vars, $edit = false)
    {
        $rules = [
            'staff_id' => [
                ...
                'belongs' => [
                    'rule' => [[$this, 'validateStaffInCompany'], $this->ifSet($vars['company_id'])],
                    'message' => $this->_('SupportManagerStaff.!error.staff_id.belongs')
                ],
                'duplicate' => [
                    'rule' => [[$this, 'validateDuplicateStaff'], $this->ifSet($vars['company_id'])],
            ...
            'departments' => [
                'exists' => [
                    'rule' => [[$this, 'validateDepartmentExists'], $this->ifSet($vars['company_id'])],
